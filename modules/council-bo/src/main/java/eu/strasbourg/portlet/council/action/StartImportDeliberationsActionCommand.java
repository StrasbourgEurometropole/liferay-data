...
public class StartImportDeliberationsActionCommand implements MVCActionCommand {
	...
	private File deliberationsFile = null;
	...
	public boolean processAction(ActionRequest request, ActionResponse response) throws PortletException {
		...
		deliberationsFile = uploadRequest.getFile("deliberations-file");
		...
		if (deliberationsFile != null) {
			...
			try {
				fileReader = new FileReader(deliberationsFile);
				...
				try {
					...
					if (chaine.length == 14 && chaine[0].equals("Identifiant_Lieu_SIG")
						...
						traitementFichier(bufferedReader);
...
//		sendMail();
	...
	public void sendMail() {
		...
		if (resultat.equals("ERREUR")) {
			...
			if (deliberationsFile != null) {
				corps += deliberationsFile.getName();
			}
			...
			corps = "L'import du fichier " + deliberationsFile.getName()
