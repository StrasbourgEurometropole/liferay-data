jQuery(function() {
	...
	// When clicking on a language
	 jQuery('body').off('click').on('click', '.lfr-translation-manager-translation', function() {
		var xmlInput = jQuery('input[name=_eu_strasbourg_portlet_edition_EditionBOPortlet_description]')[0];
		...
		var editor = tinyMCE.editors.filter(e => e.id === '_eu_strasbourg_portlet_edition_EditionBOPortlet_descriptionEditor')[0];
	...
	.on('click', '#_eu_strasbourg_portlet_edition_EditionBOPortlet_publish', function() {
		jQuery('input[name=_eu_strasbourg_portlet_edition_EditionBOPortlet_forceStatus]')[0].value
		= 'publish';
	})
	// Lors du clic sur le bouton 'dépublier', on set le champ 'forceStatus'
	.on('click', '#_eu_strasbourg_portlet_edition_EditionBOPortlet_unpublish', function() {
		jQuery('input[name=_eu_strasbourg_portlet_edition_EditionBOPortlet_forceStatus]')[0].value
		= 'unpublish';
	});
	...
	// current language
	jQuery('body').off('submit').on('submit', '#_eu_strasbourg_portlet_edition_EditionBOPortlet_fm', function() {
		var xmlInput = jQuery('input[name=_eu_strasbourg_portlet_edition_EditionBOPortlet_description]')[0];
		var editor = tinyMCE.editors.filter(e => e.id === '_eu_strasbourg_portlet_edition_EditionBOPortlet_descriptionEditor')[0];
