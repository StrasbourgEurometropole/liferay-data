...
public class SaveManifestationActionCommand
	...
	public boolean processAction(ActionRequest request, ActionResponse response)
		...
		try {
			...
			if (manifestationId == 0) {
				eventManifestation = _eventManifestationLocalService.createManifestation(sc);
			} else {
				eventManifestation = _eventManifestationLocalService.getManifestation(manifestationId);
			}
			...
			eventManifestation.setDisplayDate(displayDate);

			// Events
			List<Event> oldEvents = eventManifestation.getEvents();
			for (Event event : oldEvents) {
				_eventManifestationLocalService.deleteEventManifestation(event.getEventId(), eventManifestation);
			}
			long[] eventsIds = ParamUtil.getLongValues(request, "eventsIds");
			for (long eventId : eventsIds) {
				if (eventId > 0) {
					_eventManifestationLocalService.addEventManifestation(eventId, eventManifestation);
				}
			}
			
			_eventManifestationLocalService.updateManifestation(eventManifestation, sc);
