package eu.strasbourg.portlet.ejob.itemselector;
...
public class OfferItemSelectorView
	implements ItemSelectorView<OfferItemSelectorCriterion> {
	...
	@Reference(target = "(osgi.web.symbolicname=ejob.web)", unbind = "-")
	public void setServletContext(ServletContext servletContext) {
		this._servletContext = servletContext;
	}

	@Override
	public Class<OfferItemSelectorCriterion> getItemSelectorCriterionClass() {
		return OfferItemSelectorCriterion.class;
	}
	...
	public void renderHTML(ServletRequest servletRequest,
		ServletResponse servletResponse,
						   OfferItemSelectorCriterion itemSelectorCriterion,
		...
		List<Offer> offers = OfferLocalServiceUtil.findByKeyword(keywords,
			themeDisplay.getScopeGroupId(), (delta * cur) - delta,
			(delta * cur));

		long offersCount = OfferLocalServiceUtil.findByKeywordCount(
			keywords, themeDisplay.getScopeGroupId());

		servletRequest.setAttribute("total", offersCount);
		servletRequest.setAttribute("offers", offers);
		...
		RequestDispatcher requestDispatcher = servletContext
			.getRequestDispatcher(
				"/offer/item-selector/offer-item-selector.jsp");
