...
<section id="graveyard">
	<div class="seu-container graveyard-web">
		...
		<!-- Messages d'erreur -->
		<liferay-ui:error key="deathdates-not-valid" message="deathdates-not-valid" targetNode=".seu-error-messages" />
		<liferay-ui:error key="birthdates-not-valid" message="birthdates-not-valid" targetNode=".seu-error-messages" />
		<liferay-ui:error key="invalid-characters" message="invalid-characters" targetNode=".seu-error-messages" />
		<liferay-ui:error key="deathdates-required" message="deathdates-required" targetNode=".seu-error-messages" />
		<liferay-ui:error key="birthdates-required" message="birthdates-required" targetNode=".seu-error-messages" />
		<liferay-ui:error key="firstname-required" message="firstname-required" targetNode=".seu-error-messages" />
		<liferay-ui:error key="name-required" message="name-required" targetNode=".seu-error-messages" />
		<liferay-ui:error key="required" message="required" targetNode=".seu-error-messages" />
		...
		<c:if test="${not empty dc and not empty dc.graveyard}">
			<div class="graveyard-response rte">
				<c:if test="${dc.err eq 1}">
					<p>${graveyardResponse.err_desc}</p>
				</c:if>
				<c:if test="${dc.err eq 0}">
					<c:if test="${not empty dc.erreur}">
						<p>${dc.erreur}</p>
					</c:if>
					<c:if test="${empty dc.erreur}">
						<p>
					    	<c:choose>
				...
				<c:if test="${dc.err eq 0 && dc.searchContainer.total gt 0}">
					<!-- Liste des résultats -->
