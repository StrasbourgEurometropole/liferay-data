...
public class PetitionServiceUtil {
	...
	private static ServiceTracker<PetitionService, PetitionService>
		_serviceTracker;

	static {
		Bundle bundle = FrameworkUtil.getBundle(PetitionService.class);

		ServiceTracker<PetitionService, PetitionService> serviceTracker =
			new ServiceTracker<PetitionService, PetitionService>(
				bundle.getBundleContext(), PetitionService.class, null);
