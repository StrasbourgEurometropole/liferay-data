 ...
 * @see eu.strasbourg.service.project.service.persistence.ProjectUtil
 * @generated
 */
@ProviderType
public class ProjectPersistenceImpl extends BasePersistenceImpl<Project>
	...
	public List<Project> filterFindByGroupId(long groupId, int start, int end,
		...
		String sql = InlineSQLHelperUtil.replacePermissionCheck(query.toString(),
				Project.class.getName(), _FILTER_ENTITY_TABLE_FILTER_PK_COLUMN,
				groupId);
	 ...
	 */
	@Override
	public Project[] filterFindByGroupId_PrevAndNext(long projectId,
		long groupId, OrderByComparator<Project> orderByComparator)
		throws NoSuchProjectException {
		if (!InlineSQLHelperUtil.isEnabled(groupId)) {
			return findByGroupId_PrevAndNext(projectId, groupId,
				orderByComparator);
		}
		...
		try {
			...
			array[0] = filterGetByGroupId_PrevAndNext(session, project,
					groupId, orderByComparator, true);

			array[1] = project;

			array[2] = filterGetByGroupId_PrevAndNext(session, project,
	...
	protected Project filterGetByGroupId_PrevAndNext(Session session,
		Project project, long groupId,
		OrderByComparator<Project> orderByComparator, boolean previous) {
		StringBundler query = null;
	...
	protected Project toUnwrappedModel(Project project) {
		...
		projectImpl.setImageWidth(project.getImageWidth());
		projectImpl.setImageHeight(project.getImageHeight());
	...
	private static final String _FILTER_ENTITY_TABLE_FILTER_PK_COLUMN = "project.projectId";
	private static final String _FILTER_SQL_SELECT_PROJECT_WHERE = "SELECT DISTINCT {project.*} FROM project_Project project WHERE ";
	private static final String _FILTER_SQL_SELECT_PROJECT_NO_INLINE_DISTINCT_WHERE_1 =
		"SELECT {project_Project.*} FROM (SELECT DISTINCT project.projectId FROM project_Project project WHERE ";
	private static final String _FILTER_SQL_SELECT_PROJECT_NO_INLINE_DISTINCT_WHERE_2 =
		") TEMP_TABLE INNER JOIN project_Project ON TEMP_TABLE.projectId = project_Project.projectId";
	private static final String _FILTER_SQL_COUNT_PROJECT_WHERE = "SELECT COUNT(DISTINCT project.projectId) AS COUNT_VALUE FROM project_Project project WHERE ";
	private static final String _FILTER_ENTITY_ALIAS = "project";
	private static final String _FILTER_ENTITY_TABLE = "project_Project";
	private static final String _ORDER_BY_ENTITY_ALIAS = "project.";
	private static final String _ORDER_BY_ENTITY_TABLE = "project_Project.";
