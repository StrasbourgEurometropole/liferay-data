...
public class PricePersistenceImpl extends BasePersistenceImpl<Price>
	...
	public List<Price> findByUuid(String uuid, int start, int end,
		...
		if (list == null) {
			...
			query.append(_SQL_SELECT_PRICE_WHERE);
	...
	public int countByUuid(String uuid) {
		...
		if (count == null) {
			...
			query.append(_SQL_COUNT_PRICE_WHERE);
	...
	public Price remove(Serializable primaryKey) throws NoSuchPriceException {
		...
		catch (NoSuchPriceException nsee) {
			throw nsee;
		}
		catch (Exception e) {
			throw processException(e);
	...
	public Price updateImpl(Price price) {
		...
		if (isNew || !PriceModelImpl.COLUMN_BITMASK_ENABLED) {
			finderCache.clearCache(FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION);
		}
	...
	public Map<Serializable, Price> fetchByPrimaryKeys(
		...
		try {
			...
			for (Price price : (List<Price>)q.list()) {
	...
	public int countAll() {
		...
		if (count == null) {
			...
			try {
				...
				Query q = session.createQuery(_SQL_COUNT_PRICE);
