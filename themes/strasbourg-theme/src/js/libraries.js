 ...
 */
function cl(p, fct){
    if(typeof p == 'undefined' || !debugCustom){return false;}
        ...
        (function ($) {
            $('[data-dot]').each(function(){
                ...
                if(lines == ''){
                    ...
                    height += 2; // Pour être sur de passer tous les arrondis (1 suffit pas)
...
var environment, environmentChanged, windowWidth, windowHeight, viewport, testEnvironment;
...
// Un jour peut être, ce script servira à réajuster les dimensions des gmap quand on les print. Il faudrait stocker les data leaflet de chaque map dans leur div receptrice dans le dom, pour pouvoir le récup ici et trigger l'invalidSize en JS
...
//         if(printReady){
//             i.map.leaflet.instance.invalidateSize();
//         }
//     };
//     var afterPrint = function() {
//         console.log('Functionality to run after printing');
//         if(typeof i.map != 'undefined' && i.map){
//             i.map.leaflet.instance.invalidateSize();
//         }
//     };
 ...
 * @param {*} conf 
 */
function smag_getSources(source, conf){
    conf.source_tous = source; // Table for category 'tous'
    // Build tables for each category
    source.forEach(function(element, index) {
        var category = element.category;
 ...
 * @param {Object} conf           - Configuration object
 */
function smag_majItems(category, conf){
    var source = conf['source_'+category] !== undefined ? conf['source_'+category] : [],
        clones = [];
    source.forEach(function(element, index) {
        var item_category = element.category; // Get element category
        ...
        var clone_html = smag_setTemplateInfos(element, item_category, $template, conf); // Set informations from source element inside the cloned template
        if(element.is_Big){
            clones.unshift(clone_html);
        }else{
            clones.push(clone_html);
 ...
 * @return {jQueryObject}
 */
function smag_setTemplateInfos(infos, category, $template, conf){
    var $new_template = $template.clone();
    if(infos.picture != undefined && infos.picture != '' && conf.hasPicture_class != undefined){
        $new_template.addClass(conf.hasPicture_class);
    }
    ...
    if( (infos.category == "facebook" | infos.category == "twitter" ) && (infos.picture != undefined && infos.picture != '')){
        $new_template.find('.smag-content').attr('data-dot', 3);
    }
    var clone = $new_template[0].outerHTML;
 ...
 * @param {Array} elements - Array of all elements to add
 */
function smag_addToSlider(conf, elements){
    var nodeStringToAdd = '<div class="'+conf.pages_class+'">';
    var itemSeparator = 8;
    if(elements[0].indexOf('seu-big') != -1){
        itemSeparator = 7;
 ...
 * @param {Object} conf - Configuration object
 */
function smag_removeAllItems(conf){
    conf.$slider.find('.'+conf.pages_class).remove();
}
 ...
 * @param {Object} conf - Configuration object
 */
function smag_buildSlider(conf){
    conf.$slider.addClass('owl-carousel').owlCarousel({
        margin : 20,
        items: 1,
        smartSpeed: 800,
        onInitialized: function(e){
            smag_manageNavigationDisplay(e, conf.$prev, conf.$next);
            smag_attachCustomNavEvents(conf.$slider, conf.$prev, conf.$next);
        },
        onTranslate : function(e){
            smag_manageNavigationDisplay(e, conf.$prev, conf.$next);
        }
 ...
 * @param {Object} conf - Configuration object
 */
function smag_destroySlider(conf){
    conf.$slider.trigger('destroy.owl.carousel');
}
 ...
 * @param {string} category - Catégorie à afficher
 */
function smag_megaSlider(slider, category){
    smag_destroySlider(slider);
    smag_removeAllItems(slider);
    smag_majItems(category, slider);
    smag_buildSlider(slider);
    dot();
}
// (function ($) {
    ...
    //     }
    // }
    function smag_manageNavigationDisplay(e, $prev, $next){ // Ne fonctionne pas avec loop: true
        if(e.item.index == 0){
            $prev.addClass('disabled');
        }else{
            $prev.removeClass('disabled');
    ...
    function smag_updatePagination(e, $prev, $next){
    ...
    function smag_updateSliderText($slider, $title, $description, $link, index){
    ...
    function smag_attachCustomNavEvents($slider, $prev, $next){
    ...
    function smag_attachCustomPlayEvents($slider, $play){
    ...
    function smag_fixRestartAutoplayAfterNavigationClick($slider, $prev, $next, $dots, $play){
...
/**
 * @name formatPublicationDate
 * @description Mise en forme d'une date à partir d'un timestamp, peut retourner il y a X minutes, ou X heures, ou une date précise en fonction de la date actuelle
 * @param {Integer} timestamp
 * @returns {string} Date formatée
 */
function formatPublicationDate(timestamp){
    var a = new Date(timestamp * 1000);
    var n = Date.now();
    var mn_spent = ((n - a)/1000) / 60;
    var time ="";
    if(mn_spent < 60){
        time = 'Il y a ' + Math.floor(mn_spent) + ' min';
    }else if(mn_spent < 1440){
        time = 'Il y a ' + Math.floor(mn_spent / 60) + ' heures';
    }else{
        ...
        var time = 'Le ' + date + ' ' + month + ' ' + year + ' à ' + hour + 'h' + min;
    }
    return time;
}
...
/**
 * @name populateComment
 * @description Met en forme un template de commentaire à partir d'un objet commentaire
 * @param {Object} comment Objet commentaire à styliser
 * @returns {string} Template de commentaire formaté
 */
function populateComment(comment, c){
    cl('CMNT: populating comment: ' + comment);
    var comment_tpl;
    if(!comment.deleted){ // Si commentaire pas supprimé
        comment_tpl = c.tpl.empty;

        // Gestion des attributs
        var attributs = '';
        attributs += comment.isUserComment ? 'data-owner="true" ' : '';
        attributs += comment.canBeReported ? 'data-reportable="true" ' : '';
        attributs += comment.userLike ? 'data-liked="true" ' : '';
        attributs += comment.parent_id !== 0 ? 'data-parent-id="'+comment.parent_id+'" ' : '';
        attributs += 'data-comment-id="'+comment.comment_id+'" ';
        comment_tpl = comment_tpl.replace('##attributs##=""', attributs);

        // Gestion des classes
        var classes ='';
        classes += comment.parent_id !== 0 ? c.answer_cmnt_class : c.root_cmnt_class;
        comment_tpl = comment_tpl.replace('##classes##', classes);

        // Mise en forme du name et remplacement dans le tpl
        var author = '';
        if(comment.firstname !== undefined && comment.firstname !== ''){
            author += comment.firstname;
        }
        if(comment.lastname !== undefined && comment.lastname !== ''){
            author += ' ' + comment.lastname.slice(0,1) + '.';
        }
        comment_tpl = comment_tpl.replace('##author##', author);


        // Compteur de like
        var likes_count = '0';
        if(comment.likes_count !== undefined && comment.likes_count !== ''){
            likes_count = comment.likes_count;
        }
        comment_tpl = comment_tpl.replace('##likes_count##', likes_count);

        // Image user
        var picture = '';
        var default_picture = comment_tpl.slice(comment_tpl.indexOf('data-default-picture'));
        default_picture = default_picture.replace('data-default-picture="', '');
        default_picture = default_picture.slice(0, default_picture.indexOf('">'));
        if(comment.picture !== undefined && comment.picture !== ''){
            picture = comment.picture;
        }else{
            picture = default_picture;
        }
        comment_tpl = comment_tpl.replace('##picture##', picture);

        // Liens vers la page de l'user
        var user_page = '#';
        if(comment.user_page !== undefined && comment.user_page !== ''){
            user_page = comment.user_page;
        }
        comment_tpl = comment_tpl.replace('##user_page##', user_page);

        // Commentaire
        var comment_content = '';
        var caracter_limit = 120; // 2 lignes max pour "voir la suite"
        if(comment.comment_content !== undefined && comment.comment_content !== ''){
            if(comment.comment_content.length > 120){
                comment_content += comment.comment_content.slice(0, 120);
                comment_content += '<span class="cmnt__more">... lire la suite</span>';
                comment_content += '<span class="cmnt__more-content">'+comment.comment_content.slice(120)+'</span>';
                comment_content += '<div class="cmnt__less">lire moins</div>';
            }else{
                comment_content = comment.comment_content;
            }
        }
        comment_tpl = comment_tpl.replace('##comment_content##', comment_content);

        // Mise en forme de la date de publication + insertion
        var date_publication = '';
        if(comment.date_publication !== undefined && comment.date_publication !== ''){
            date_publication = formatPublicationDate(comment.date_publication);
        }
        comment_tpl = comment_tpl.replace('##date_publication##', date_publication);

    }else{
        comment_tpl = c.tpl.message;
        comment_tpl = comment_tpl.replace('##webmaster_message##', 'Ce contenu a été supprimé').replace('##date_publication##', formatPublicationDate(comment.date_deleted));
 ...
 * @description Affichage d'une liste de commentaire depuis un object. Fonction chargée au lancement de la page grâce à l'objet comment_datas présent dans comment.tpl
 * @param {Object} comment_datas Object comportant la liste de tous les commentaires à charger
 */
function loadAllComments(comment_datas, c){
    cl('CMNT: Loading all comments from datas: ' + comment_datas);
    c.$root.trigger('cmnt:loading');
    var logged = false;
    if(comment_datas.isUserLogged !== undefined){
        logged = comment_datas.isUserLogged;
    };
    $.each(comment_datas.comments, function(i, comment){
        c.$comment_receiver.append(populateComment(comment, c));
        if(comment.answers !== false){
            $.each(comment.answers, function(i, answer){
                c.$comment_receiver.append(populateComment(answer, c));
            });
        }
    });
    c.$root.trigger('cmnt:loaded');
    displayComments('minimal', '', c);
}

/**
 * @name loadUniqueComment
 * @description Création d'un commentaire unique
 * @param {*} comment Object commentaire
 */
function loadUniqueComment(comment, c){
    cl('CMNT: Ajout d\'un commentaire unique');
    c.$root.trigger('cmnt:loading');
    if(comment.parent_id === 0){
        cl('CMNT: Nouveau commentaire ajout au début de la liste');
        // Nouveau commentaire => ajout au début de la liste
        c.$comment_receiver.prepend(populateComment(comment, c));
    }else{
        // Réponse ==> ajout à la fin de la liste sous le parent
        cl('CMNT: Commentaire est une réponse, ajout à la fin de la liste sous le parent');
        if(c.$each_cmnt_answer.filter('[data-parent-id="'+comment.parent_id+'"]').length){ // S'il y a déjà des réponses
            c.$each_cmnt_answer.filter('[data-parent-id="'+comment.parent_id+'"]').last().after(populateComment(comment, c));
        }else{
            c.$each_cmnt.filter('[data-comment-id="'+comment.parent_id+'"]').after(populateComment(comment, c));
        }
    }
    c.$root.trigger('cmnt:loaded');
}
 ...
 * @description Permet de gérer l'affichage des commentaires, d'en afficher plus/moins
 * @param {string} type minimal|comment|answer Minimal: Affiche 3 comment max et 3 answer max.  Comment: Affiche tous les comments. Answer: Affiche toutes les réponses d'un commentaire
 * @param {int} parent_id Optionnel, va de paire avec le type 'answer', représente l'id du commentaire parent
 */
function displayComments(type, parent_id, c){
    c.$root.trigger('cmnt:loading');
    type = type || 'minimal';
    parent_id = parent_id || 0;

    /**
     * @description Affiche les 3 premières réponses d'un commentaire donné, et si + de 3, ajoute un bouton "voir plus"
     * @param {int} parent_id
     */
    function limitAnswers(parent_id){
        c.$each_cmnt_answer.filter('[data-parent-id="'+parent_id+'"]').slice(0,3).show();
        $('.answ-display-more').remove();
        if(c.$each_cmnt_answer.filter('[data-parent-id="'+parent_id+'"]').length > 3){
            c.$each_cmnt_answer.filter('[data-parent-id="'+parent_id+'"]').eq(3).after(c.tpl.more_answers.replace('##parentid##', parent_id));
        }
    }

    if(type == 'minimal'){ // Affiche 3 comment et 3 réponses max
        cl('CMNT: Affichage 3 comments et 3 réponses max');
        c.$each_cmnt.hide();
        c.$each_cmnt_root.add(c.$each_cmnt_message).slice(0,3).each(function(i, el){
            $(this).show();
            var current_parent_id = $(this).attr('data-comment-id');
            limitAnswers(current_parent_id);
        });
        if(c.$each_cmnt_root.length > 3 ){ // Si + de 3 commentaires le bouton affiche + de comment
            cl('CMNT: Plus de 3 commentaires, affichage du bouton + de commentaires');
            c.$display_more_cmnt.off('click').on('click', function(){
                displayComments('comment', '', c);
            })
                .removeClass('less')
                .find('.btn-text').text('Plus de commentaires')
                .show();
        }else{
            c.$display_more_cmnt.hide();
        }


    }else if(type == 'comment'){ // Affiche tous les comments 3 réponses max
        cl('CMNT: Affiche tous les comments, 3 réponses max');
        c.$each_cmnt_root.each(function(i, el){
            $(this).show();
            var current_parent_id = $(this).attr('data-comment-id');
            limitAnswers(current_parent_id);
        });
        c.$display_more_cmnt.off('click').on('click', function(){ // le bouton affiche - de comment
            displayComments('minimal', '',  c);
        })
        .addClass('less')
        .find('.btn-text').text('Moins de commentaires');


    }else if(type == 'answer'){ // Affiche toutes les réponse d'un commentaire précis
        cl('CMNT: Affiche toutes les réponses du commentaire ' + parent_id);
        if(parent_id !== 0){
            c.$each_cmnt_answer.filter('[data-parent-id="'+parent_id+'"]').show();
        }else{
            cl('CMNT: Error, missing "parent_id" argument', 'error');
        }
    }else{
        cl('CMNT: Error, wrong value for "type" argument', 'error');
    }
    c.$root.trigger('cmnt:loaded');
}

/**
 * @name majCommentCount
 * @description Met à jour le compteur de commentaires
 */
function majCommentCount(c){
    var count = c.$each_cmnt_root.filter(':not(.message)').length;
    c.$cmnt_count.html(count);
}
...
(function(root, factory) {
	...
	$.extend(Selectize.prototype, {
		 ...
		 */
		refreshValidityState: function() { // Remove cette partie qui fuck la validation
...
function manageNavigationDisplay(e, $prev, $next) { // Ne fonctionne pas avec loop: true
	var current_index = getRealCurrentIndex(e);
	if (current_index == 1) {
		$prev.addClass('disabled');
	} else {
		$prev.removeClass('disabled');
	}
	if ((current_index + (e.page.size - 1 )) == e.item.count) {
		$next.addClass('disabled');
	} else {
		$next.removeClass('disabled');
	}
...
/**
 * @description Fixe les problèmes de wrong index dans plusieurs conditions, notamment avec loop = true
 * @param {*} e event Owl
 */
function getRealCurrentIndex(e){
	total = e.item.count;
...
/**
 * @name updatePager
 * @param {event} e - event from owl carousel
 * @param {Object} s - Master slider object
 */
function updatePager(e, s) {
	var current, total, prev, next;
	total = e.item.count;
	current = getRealCurrentIndex(e);
	if (current == 1) {
		prev = total;
	} else {
		prev = current - 1;
	...
	s.main.pager.$current.html(current);
	s.main.pager.$total.html(total);
	s.main.pager.$prev.html(prev);
	s.main.pager.$next.html(next);
}


function buildDots($li, $dots, $dot_template){
	$li.each(function(index, element){
		var $dot = $dot_template.clone();
		index ++;
		$dot.html(index).attr('data-slider-index', (index - 1));
		$dots.append($dot);
	});
}

function majActiveDot(e, s){
	var real_current = getRealCurrentIndex(e) - 1;
	$current_dot = s.main.dot.$dot.filter('[data-slider-index="'+real_current+'"]');
	s.main.dot.$dot.removeClass('active');
	$current_dot.addClass('active');
}


/**
 * @description Maj les infos textes dynamique par dessus un slider
 * @param {*}  s 
 */
function updateSliderText(e, s) {
	var $currentSlide = s.main.$slider.find('.owl-item:eq(' + e.item.index + ') li > div');
	var infos_to_change = [];
	if(s.main.text.infos.length){
		s.main.text.infos.forEach(function(info, index){
			if($currentSlide.attr(info.data) !== undefined && $currentSlide.attr(info.data) != '' && $currentSlide.attr(info.data) != ' '){
				infos_to_change.push(index);
			}
		});
		if(infos_to_change.length > 0){
			s.main.text.infos.forEach(function(info, index){
				var $receiver = info.$selector;
				var data = $currentSlide.attr(info.data);
				if(info.type == "text"){
					...
					$receiver.html(data);
				}else if(info.type == "href"){
					$receiver.attr('href', data);
				}else if(info.type == "background"){
					$receiver.css('background-image',data);
				}
			});
			s.main.text.$root.show();
		}else{
			s.main.text.$root.hide();
		}	
	}
}

/**
 * @description Attache les écouteurs sur les arrows
 * @param {*} s 
 */
function attachCustomArrowsEvents(s, $slider, $prev, $next) {
	cl('Slider Addons: attaching custom nav events');
	$prev.add($next).off('click');
	$prev.on('click', function () {
		if(!s.can_animate) return;
		if(s.autoplay !== undefined && s.autoplay.has_autoplay)destroyAutoPlayInterval(s);
		//s.can_animate = false;
		$slider.trigger('prev.owl.carousel');
	});
	$next.on('click', function () {
...
/**
 * @description start l'autoplay du slider
 * @param {*} s 
 */
function buildAutoPlayInterval(s){
	s.autoplay.$playpause.removeClass('play').addClass('pause');
	s.autoplay.interval = setInterval(function(){
		s.$slider.trigger('next.owl.carousel');
	}, s.autoplay.timeout);
}

/**
 * @description Stop l'autoplay du slider
 * @param {*} s 
 */
function destroyAutoPlayInterval(s){
	clearInterval(s.autoplay.interval);
	s.autoplay.$playpause.removeClass('pause').addClass('play');
}

/**
 * @description Attache les écouteurs sur le playpause
 * @param {Object} s Objet Maître Slider
 */
function attachCustomPlayEvents(s) {
	cl('Slider addons: Attaching custom play events on:');
	cl({"slider": s.$slider, "playpause": s.autoplay.$playpause});
	if(s.autoplay.start_autoplay) buildAutoPlayInterval(s);
	s.autoplay.$playpause.on('click', function () {
		if (s.autoplay.$playpause.hasClass('pause')) {
			cl('Slider addons: pause triggered');
			destroyAutoPlayInterval(s);
		} else {
			cl('Slider addons: play triggered');
			buildAutoPlayInterval(s);
		}
	});
}
...
var masterSliderFull, domSliderMain, sliderMain, domSlideNpop, slideNpop, domSliderThumbnail, sliderThumbnail;
(function ($) {
	
	/**
	 * @name masterSliderFull
	 * @description fonction parente, qui gère le flux d'appel des autres fonctions
	 */
	masterSliderFull = function(s){
		domSliderMain(s);

		// Slider Thumbnail /!\ désactive les DOTS et l'autoplay
		if (s.thumbnail.has_thumbnail) domSliderThumbnail(s);

		// Slider slideNpop
		if(s.slideNpop.has_slideNpop) domSlideNpop(s);
		s.$root.trigger('sliderfull:domready', [{slider: s}]);

		sliderMain(s);
		
		if(s.slideNpop.has_slideNpop) slideNpop(s);
		if(s.thumbnail.has_thumbnail) sliderThumbnail(s);

	};
	
	/**
	 * @name domSliderMain
	 * @description Création du DOM nécessaire au fonctionnement du slider ppal
	 * @argument {Object} s Objet maître de configuration du slider
	 */
	domSliderMain = function(s){
		// // SafetyBelt
		if (s.thumbnail.has_thumbnail && s.slideNpop.has_slideNpop && s.slideNpop.slide_slider_type == "new") cl('Slider Full: Slider thumbnail incompatible avec SlideNpop avec un slider new', 'error');

		// Classe sur le slider pour gérer l'affichage de l'UI
		s.main.$slider.addClass('owl-carousel');
		if (!s.main.autoplay.has_autoplay) s.main.$root.addClass('no-autoplay');
		if (!s.main.text.has_text) s.main.$root.addClass('no-text');
		if (!s.main.dot.has_dot) s.main.$root.addClass('no-dot');
		if (!s.main.pager.has_pager) s.main.$root.addClass('no-pager');
		if (!s.main.arrow.has_arrow) s.main.$root.addClass('no-arrow');

		// Build les dots avant le slider + attach event
		if (s.main.dot.has_dot && !s.thumbnail.has_thumbnail) {
			buildDots(s.main.$slider_item, s.main.dot.$dots, s.main.dot.$dot);
			s.main.dot.$dot = s.main.dot.$dots.find('.'+s.main.name+'__dot');
			s.main.dot.$dot.on('click', function () {
				cl('Slider full: dot cliked');
				if (!s.main.can_animate) return;
				destroyAutoPlayInterval(s.main);
				var index = $(this).attr('data-slider-index');
				s.main.$slider.trigger('stop.owl.carousel');
				s.main.$slider.trigger('to.owl.carousel', [index]);
			});
		}
	};
	...
	/**
	 * @name sliderMain
	 * @description initialisation du slider main
	 */
	sliderMain = function (s) {

		// Build Slider
		s.main.$slider.owlCarousel($.extend(s.main.conf, {
			onInitialized: function (e) {
				if (s.main.arrow.has_arrow) {
					attachCustomArrowsEvents(s.main, s.main.$slider, s.main.arrow.$prev, s.main.arrow.$next);
					if(typeof s.main.conf.loop == "undefined" | !s.main.conf.loop)
					manageNavigationDisplay(e, s.main.arrow.$prev, s.main.arrow.$next);
				}
				if (s.main.autoplay.has_autoplay) attachCustomPlayEvents(s.main);
				if (s.main.pager.has_pager) updatePager(e, s);
				if (s.main.dot.has_dot && !s.thumbnail.has_thumbnail) majActiveDot(e, s);
				if (s.main.text.has_text) updateSliderText(e, s);
				if (s.main.animation.css_custom) {
					s.main.$slider.find('.owl-item').on('webkitAnimationEnd animationEnd MSAnimationEnd', function () { // Event de fin d'animation CSS des slides
						s.main.$root.removeClass('animating');
					});
				}
				s.main.$root.addClass('loaded');
				if (s.slideNpop.has_slideNpop && s.slideNpop.slide_slider_type == 'same') {
					s.main.$slider.find('.owl-item li').on('click', function () {
						if (s.main.autoplay.has_autoplay) destroyAutoPlayInterval(s.main);
						s.main.$root.addClass('opened');
						s.main.$slider.trigger('refresh.owl.carousel');
					});
				}
			},
			onTranslate: function (e) {
				s.main.can_animate = false;
				s.main.$root.addClass('animating');
				if (s.main.arrow.has_arrow && ( typeof s.main.conf.loop == "undefined" | !s.main.conf.loop) ) manageNavigationDisplay(e, s.main.arrow.$prev, s.main.arrow.$next);
				if (s.main.pager.has_pager) updatePager(e, s);
				if (s.main.dot.has_dot && !s.thumbnail.has_thumbnail) majActiveDot(e, s);
				// if (s.thumbnail.has_thumbnail) {
				// 	var index = getRealCurrentIndex(e) - 1;
				// 	s.thumbnail.$slider.trigger('to.owl.carousel', [index]);
				// }
				// @todo, trigg event custom pour pouvoir se plugguer dessus dans slider.conf
			},
			onTranslated: function (e) {
				s.main.can_animate = true;
				if (!s.main.animation.css_custom){
					s.main.$root.removeClass('animating');
					if (s.main.text.has_text) updateSliderText(e, s);
				}else if(s.main.text.has_text){
					setTimeout(function(){
						if(isEdge || isIE){
							s.main.$root.removeClass('animating');
						}
						updateSliderText(e, s);
					}, s.main.animation.timeout);
				}
			}
		}));
	...
	/**
	 * @name domSliderThumbnail
	 * @description construction du dom requis pour le slider Thumbnail
	 */
	domSliderThumbnail = function(s){
		var slider_name = s.thumbnail.name;
	
		// Duplicate le slider full en changeant ses noms de classes
		var rexp = new RegExp(s.main.name, "g");
		var new_slider = s.$root.html().replace(rexp, slider_name);

		// Prepare la div qui va recevoir le nouveau slider
		s.$root.addClass('slider-thumbnail').append('<div class="'+slider_name+'"></div>');
		s.thumbnail.$root = s.$root.find('.'+slider_name);
		$($(new_slider).html()).removeClass('no-arrow no-pager no-dot no-text no-autoplay').appendTo(s.thumbnail.$root);
	
		s.thumbnail = $.extend({
			$slider: s.thumbnail.$root.find('.'+slider_name+'__slider'),
			$prev: s.thumbnail.$root.find('.'+slider_name+'__arrow--prev'),
			$next: s.thumbnail.$root.find('.'+slider_name+'__arrow--next'),
		}, s.thumbnail);
	};

	/**
	 * @name sliderThumbnail
	 * @description initialisation du slider thumbnail
	 */
	sliderThumbnail = function(s){
	
		// Build Slider
		s.thumbnail.$slider.owlCarousel($.extend(s.thumbnail.conf, {
			onInitialized: function(e){
				attachCustomArrowsEvents(s.thumbnail, s.thumbnail.$slider, s.thumbnail.$prev, s.thumbnail.$next);
				manageNavigationDisplay(e, s.thumbnail.$prev, s.thumbnail.$next);
				s.thumbnail.$slider.find('.owl-item li').on('click', function(){
				...
				s.thumbnail.$root.addClass('loaded');
			},
			onTranslate: function (e) {
				s.thumbnail.can_animate = false;
				if(s.main.autoplay.has_autoplay) destroyAutoPlayInterval(s.main);
				var index = getRealCurrentIndex(e) - 1;
				s.main.$slider.trigger('to.owl.carousel', [index]);
				manageNavigationDisplay(e, s.thumbnail.$prev, s.thumbnail.$next);
	...
	/**
	 * @name domSlideNpop
	 * @description Création du dom nécessaire au slideNpop
	 */
	domSlideNpop = function(s){
		s.$root.addClass(s.slideNpop.slide_slider_type+ '-slider slider-slideNpop');
		if (s.slideNpop.slide_slider_type != 'same') {
			var slider_name = s.slideNpop.slide_slider_type == 'grid' ? s.name + '-slideNpopGrid' : s.slideNpop.name;

			// Duplicate le slider full en changeant ses noms de classes
			var rexp = new RegExp(s.main.name, "g");
			if(s.slideNpop.slide_slider_type == 'new'){
				var new_slider = s.$root.html().replace(rexp, slider_name);
				$(new_slider).appendTo(s.$root);
				s.slideNpop.$root = s.$root.find('.'+slider_name);
			}else{
				var new_slider = s.main.$slider[0].outerHTML.replace(rexp, slider_name).replace(/owl-carousel/g, '');
				s.slideNpop.$root = s.$root.append('<div class="'+slider_name+'"></div>');
				$(new_slider).removeClass('no-arrow no-pager no-dot no-text no-autoplay').appendTo(s.slideNpop.$root);
			}

			s.slideNpop = $.extend({
				$slider: s.slideNpop.$root.find('.' + slider_name + '__slider'),
				$prev: s.slideNpop.$root.find('.' + slider_name + '__arrow--prev'),
				$next: s.slideNpop.$root.find('.' + slider_name + '__arrow--next')
			}, s.slideNpop);
		}
	};
	...
	/**
	 * @name slideNpop 
	 * @description initialisation du slider slide N pop
	 */
	slideNpop = function (s) {
		if (s.slideNpop.slide_slider_type != 'same') {

			if (s.slideNpop.slide_slider_type == 'new') {
				// Build Slider
				s.slideNpop.$slider.owlCarousel($.extend(s.slideNpop.conf, {
					onInitialized: function (e) {
						attachCustomArrowsEvents(s.slideNpop, s.slideNpop.$slider, s.slideNpop.$prev, s.slideNpop.$next);
						s.slideNpop.$slider.find('.owl-item li').on('click', function () {
							if (s.main.autoplay.has_autoplay) destroyAutoPlayInterval(s.main);
							var index = $(this).attr('data-index');
							s.main.$root.addClass('opened');
							s.main.$slider.trigger('to.owl.carousel', [index]);
						});
						s.slideNpop.$root.addClass('loaded');
					}
				}));
			} else {
				// Gere l'affichage du slider au clic sur un élément de la grille
				s.slideNpop.$slider.find('li').on('click', function () {
					s.$root.trigger('slideNpop:grid:open', [{index: $(this).attr('data-index')}]);
				});
		...
		} else if (s.slideNpop.slide_slider_type == 'same') {

		}


		// Ajoute un bouton close au slider Full
		s.slideNpop.$slider_pop_close.on('click', function () {
			s.main.$root.removeClass('opened');
			s.main.$slider.trigger('refresh.owl.carousel');
		});
		s.main.$root.append(s.slideNpop.$slider_pop_close);
	};
