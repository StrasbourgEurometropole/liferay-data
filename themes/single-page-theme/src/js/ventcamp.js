...
Ventcamp = {
                                ...
                                if (!$el.closest(".coundown").length) {
                                    if (typeof $.fn.countTo == "function") {
                                        if ($el.find(".count").length)
                                            $el.find(".count").countTo();
                                    } else {
                                        this.log(
                                            "Can't find jQuery.countTo function"
                                        );
                                    }
                                }
                            }
        ...
        if ($dropdownItem.length) {
            $dropdownItem.each(function() {
                var $this = $(this),
                    ...
                    headerHeight = $dropdownItem
                        .closest("header.header.header-black")
                        .outerHeight();
    ...
    tabNavToSelect: function() {
        ...
        $nav.each(function() {
            var $this = $(this),
                ...
                $field = $(
                    '<span class="nav-current">' + $active.html() + "</span>"
                );
    ...
    formInit: function() {
        ...
        validateOptions = {
            ...
            submitHandler: function(form) {
                ...
                if ($button.length) {
                    $button.append(
                        '<span class="loading fa fa-refresh"></span>'
                    );
                } else if ($input) {
                ...
                $(this).on("submit", function(event) {
                    ...
                    if ($(this).find("[name=NewsletterName]").length > 0) {
                        ...
                        (fullname = $fullnameField
                            .val()
                            .split(" ")), (first_name = // Gwt Full name and split it by space
                            fullname[0]), (last_name = fullname[1]); // First part is the first name // Second part is Last name
            ...
            $(".map").each(function() {
                ...
                function successCallback(data) {
                    if (typeof data == "object") {
                        ...
                        if (
                            data.markerImagePath &&
                            typeof data.markerImagePath == "string"
                        )
                            icon = data.markerImagePath;

                        if (
                            data.markerAddress &&
                            typeof data.markerAddress == "string"
                        )
                            address = data.markerAddress;
                        else if (
                            data.markerLatLng &&
                            typeof data.markerLatLng == "object"
                        )
                ...
                $container.masonry({
                    ...
                    columnWidth: function(containerWidth) {
                        if (containerWidth > 720) {
                            $container
                                .removeClass("width400")
                                .removeClass("width720");
                            return containerWidth / 4;
                        } else if (containerWidth > 400) {
                            $container
                                .addClass("width720")
                                .removeClass("width400");
                            if ($container.is(".speakers"))
            ...
            $(".gallery.light-slider").each(function() {
                ...
                if ($imgSLider.length && $descrSlider.length) {
                    function countImgMinHeight($slide) {
                        ...
                        img.onload = function() {
                            height =
                                this.height * $slide.outerWidth() / this.width;
    ...
    stickMenu: function() {
        var $header = $("header.header.header-black");
        ...
        if (typeof $.fn.waypoint != "undefined") {
            ...
            $("body .see").on("click", 'a[href*="#"]', function(event) {
                ...
                if ($(anchorId).length) {
                    ...
                    return false;
                } else if (anchorId.length > 1) {
                    window.location = window.homeURL + anchorId;
                }
            });
        } else {
    ...
    anchorClickHandler: function(anchorId) {
        ...
        if (!!$(anchorId).offset()) {
            var offsetTop =
                $(anchorId).offset().top -
                $("header.header.header-black").height();
    ...
    checkHeaderStatus: function() {
        if ($("header.header.header-black").length) {
            var $header = $("header.header.header-black"),
    ...
    initScheduleTabs: function() {
        $('.day-x').each(function(index) {
            $(this).attr('id', 'day' + (index + 1));
            if (index == 0) {
                $(this).addClass('active');
            }
        });
        $('[data-activate-day]').on('click', function(e) {
            e.preventDefault();
            $('[data-activate-day]').parent().removeClass('active');
            $(this).parent().addClass('active');
            var day = $(this).data('activate-day');
    ...
    initHomeLink: function() {
        $('.navigation-header a.logo').attr('href', window.homeURL);
    },
