                ...
                <div class="seu-gray-box">
                    ...
                    <#if entry.getPermanentDescription()?? && entry.offerTypeRecrutement.getTitle(locale)!="Stage">
                        <div id="permanentDescription">
                            ...
                            <p>${entry.getPermanentDescription()}</p>
                        </div>
                    </#if>
                    <div id="dureeContrat">
...
<script>
    document.getElementById("candidater").onclick = function(e){
        ...
        if(window.publikInternalId != undefined){
            window.location = "${StrasbourgPropsUtil.getPublikApiBase()}${StrasbourgPropsUtil.getPublikJobOfferApply()}?refposte=${entry.publicationId}&libposte=${entry.getPost(locale)}";
        }else{
