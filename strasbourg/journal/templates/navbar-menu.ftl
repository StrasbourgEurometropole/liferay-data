<#if menuNavigation.getSiblings()?has_content>
    ...
    <ul id="inline-account-menu" class="unstyled">
        <#list menuNavigation.getSiblings() as menu>
            <li><a id="menu-item-${menu?index}" href="${menu.link.getData()}" class="account-link ${menu.classMenu.getData()}"  <#if menu.newWindow.data == "true">target="_blank" title="${menu.data} (nouvelle fenêtre)"</#if>></a></li>
        </#list>
